...
\begin{dfnbox}{Subset, $A \subseteq B$}
	Let $A$ and $B$ be sets. We say that $A$ is a \dfntxt{subset} of $B$, denoted by $A \subseteq B$, if every element of $A$ is also an element of $B$, i.e., for all $a \in A$ we have $a \in B$.
\end{dfnbox}
...
\begin{dfnbox}{Function, Domain, Codomain, Maps, $f: A \to B$, $f(x)$}
	...
	We write $f: A \to B$ to denote that $f$ is a function from $A$ to $B$, and given $x \in A$, we write $f(x)$ to denote the unique element of $B$ that is related to $x$ by $f$. We say that $f$ \dfntxt{maps} $x$ to $f(x)$.
\end{dfnbox}

\begin{dfnbox}{Injective Function, Injection}
	Let $A$ and $B$ be sets. A function $f: A \to B$ is \dfntxt{injective}, or an \dfntxt{injection}, if no two distinct elements of $A$ are mapped to the same element of $B$ by $f$.
	\[ \forall x, y \in A : x \ne y \implies f(x) \ne f(y) \]
\end{dfnbox}
...
\begin{dfnbox}{Topology}
	Let $X$ be a set. A \dfntxt{topology} on $X$
\end{dfnbox}
